...
function uninstallAssistify(): void {
  ...
  delete (window as unknown as { ASSISTIFY_CONFIG?: unknown }).ASSISTIFY_CONFIG;
}
...
describe('autoload:false buffer-vs-boot contract', () => {
  ...
  it('pre-load reset() clears local visitor storage without injecting the script', () => {
    const key = 'assistify.aaaaaaaaaaaaaaaa.vid';
    const vid = 'anon_' + 'a'.repeat(22);
    document.cookie = `${encodeURIComponent(key)}=${encodeURIComponent(vid)}`;
    window.localStorage.setItem(key, vid);
    window.sessionStorage.setItem(key, vid);

    const handle = mount({ widgetId: 'aaaaaaaaaaaaaaaa', autoload: false });
    expect(handle.user.getVisitorId()).toBe(vid);
    handle.reset();

    expect(document.querySelector('script[src*="/widget/widget.js"]')).toBeNull();
    expect(window.localStorage.getItem(key)).toBeNull();
    expect(window.sessionStorage.getItem(key)).toBeNull();
    expect(handle.user.getVisitorId()).toBeNull();
  });

  it('pre-load reset() drops identity cached from mount({ identity })', () => {
    const handle = mount({
      widgetId: 'aaaaaaaaaaaaaaaa',
      autoload: false,
      identity: { email: 'old@user.test', userHash: 'a'.repeat(64) },
    });
    handle.reset();
    handle.load().catch(() => { /* ignore — no loader installed in this test */ });
    const script = document.querySelector<HTMLScriptElement>('script[src*="/widget/widget.js"]')!;
    expect(script.getAttribute('data-user-email')).toBeNull();
    expect(script.getAttribute('data-user-hash')).toBeNull();
  });

  it('reset() goes through the normal boot flow once the script is on the page', () => {
    const handle = mount({ widgetId: 'aaaaaaaaaaaaaaaa' });
    const script = document.querySelector<HTMLScriptElement>('script[src*="/widget/widget.js"]')!;
    const loader = installLoaderProxy();
    script.dispatchEvent(new Event('load'));
  ...
  it('an unsubscribe returned before boot cancels the buffered registration', () => {
    const handle = mount({ widgetId: 'aaaaaaaaaaaaaaaa', autoload: false });
    const cb = vi.fn();
    const unsubscribe = handle.events.on('open', cb);
    unsubscribe();
    ...
    // Both calls replay in order: the `on` registers, the `off` removes it.
    const methods = loader._queue.map((c) => c.method);
    expect(methods.indexOf('off')).toBeGreaterThan(methods.indexOf('on'));
    const offCall = loader._queue.find((c) => c.method === 'off')!;
    expect((offCall.args as unknown[])[0]).toBe('open');
    expect((offCall.args as unknown[])[1]).toBe(cb);
  });

  it('an unsubscribe created post-boot removes the listener through off()', () => {
    mount({ widgetId: 'aaaaaaaaaaaaaaaa' });
    const script = document.querySelector<HTMLScriptElement>('script[src*="/widget/widget.js"]')!;
    const loader = installLoaderProxy();
    script.dispatchEvent(new Event('load'));
